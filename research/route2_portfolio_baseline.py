...
def simulate_formal_cohort(label, signal_idx, prob, d, j, valid, status, exit_off, exit_px, entry_adj,
                           aO, rO, rH, rL, rC, Cff, dates, ids, t0, t1,
                           start_cash=1300000.0, max_positions=10, slip=0.005, fee=FEE):
    """Research-only portfolio baseline.
    Shared cash; whole shares; max_positions; T+1 entry; no duplicate holdings.
    Stop/expiry exits scheduled for next open; if that open is a locked limit-down,
    execution is postponed to the next tradable open. TP exits remain limit-order exits.
    Corporate-action economics are carried by adjusted-price ratios.
    """
    sig_by_entry = {}
    # sort each entry day's signals by probability descending
    for q in signal_idx:
        if d[q] + 1 <= t1:
            sig_by_entry.setdefault(int(d[q] + 1), []).append(int(q))
    for t in sig_by_entry:
        sig_by_entry[t].sort(key=lambda q: float(prob[q]), reverse=True)
    ...
    expo = []
    trades = []
    last_eq = float(start_cash)

    def locked_limit_down(t, s):
        if t <= 0 or not np.isfinite(rC[t-1, s]) or not np.isfinite(rO[t, s]):
            return False
        same = (np.isfinite(rH[t,s]) and np.isfinite(rL[t,s])
                and abs(rO[t,s]-rH[t,s]) < 1e-9 and abs(rO[t,s]-rL[t,s]) < 1e-9)
        return bool(same and rO[t,s] <= rC[t-1,s] * 0.905)

    def mark_value(p, t):
        s=p["s"]
        px=Cff[t,s]
        if not np.isfinite(px) or not np.isfinite(p["entry_adj"]) or p["entry_adj"] <= 0:
            return p["cost"]
        return p["shares"] * p["entry_raw"] * (px / p["entry_adj"])
    ...
    for t in range(t0, t1 + 1):
        ...
        for s in list(pos.keys()):
            ...
            q=p["q"]
            st=int(status[q])
            if st in (2,3) and locked_limit_down(t,s):
                p["xt"] = t + 1
                continue
            if st == 1 and t == int(d[q] + exit_off[q]):
                xadj=float(exit_px[q])
            else:
                xadj=float(aO[t,s]) if np.isfinite(aO[t,s]) else np.nan
            if not np.isfinite(xadj) or xadj <= 0:
                p["xt"] = t + 1
                continue
            gross_value = p["shares"] * p["entry_raw"] * (xadj / p["entry_adj"])
            proceeds = gross_value * (1 - slip) * (1 - fee - TAX)
            cash += proceeds
            trades.append({
                "cohort":label,
                "signal_date":pd.Timestamp(dates[d[q]]).strftime("%Y-%m-%d"),
                "entry_date":pd.Timestamp(dates[d[q]+1]).strftime("%Y-%m-%d"),
                "exit_date":pd.Timestamp(dates[t]).strftime("%Y-%m-%d"),
                "stock_id":str(ids[s]),
                "prob":float(prob[q]),
                "shares":int(p["shares"]),
                ...
                "net_ret":float(proceeds/p["cost"]-1),
                "status":st,
                "delayed_limit_down_days":int(t - (d[q] + exit_off[q])),
            })
            del pos[s]

        # T+1 entries, using prior-close NAV target size
        target_cash = last_eq / max_positions
        for q in sig_by_entry.get(t, []):
            ...
            if s in pos:
                continue
            raw=float(rO[t,s])
            eadj=float(entry_adj[q])
            if not np.isfinite(raw) or raw <= 0 or not np.isfinite(eadj) or eadj <= 0:
                continue
            buy_px = raw * (1 + slip)
            budget=min(target_cash,cash)
            sh=int(np.floor(budget/(buy_px*(1+fee))))
            if sh < 1:
                continue
            cost=sh*buy_px*(1+fee)
            if cost > cash + 1e-6:
                continue
            xt=int(d[q]+exit_off[q]) if status[q] > 0 else (t1+1)
            cash -= cost
            pos[s]={
                "q":int(q),"s":s,"shares":sh,"entry_raw":raw,"entry_adj":eadj,
                "cost":float(cost),"xt":xt
            }

        mv=sum(mark_value(p,t) for p in pos.values())
        last_eq=cash+mv
        eq.append(last_eq)
        expo.append(mv/last_eq if last_eq>0 else 0.0)

    eq=np.asarray(eq,float)
    return eq, pd.DataFrame(trades), float(np.mean(expo)), int(len(pos))
...
def main():
    ...
    # ───────────────── Formal Portfolio Baseline (research only; 2025 already inspected) ─────────────────
    # Cohort membership uses only same-day model scores (no future outcome):
    # D10 = daily top 10%; Top2pct = daily top 2%; P>=85 = fixed absolute score gate.
    test_idx=np.nonzero(m_test)[0]
    day_rank_pct=np.full(len(d),np.nan)
    for day in np.unique(d[test_idx]):
        qq=test_idx[d[test_idx]==day]
        if len(qq):
            order=qq[np.argsort(-prob[qq],kind="mergesort")]
            day_rank_pct[order]=(np.arange(len(order))+1)/len(order)

    cohort_masks={
        "D10_daily": m_test & (day_rank_pct <= 0.10),
        "Top2pct_daily": m_test & (day_rank_pct <= 0.02),
        "P_ge_85": m_test & (prob >= 0.85),
    }
    port_dir=os.path.join(root,"research","route2_portfolio_baseline_out")
    os.makedirs(port_dir,exist_ok=True)
    td_port=pd.DatetimeIndex(dates[t_test:t_last+1])
    port_rows=[]; all_trades=[]; curves_port={"date":td_port.strftime("%Y-%m-%d")}
    for label,mask in cohort_masks.items():
        sig=np.nonzero(mask)[0]
        eqp,trp,expp,open_end=simulate_formal_cohort(
            label,sig,prob,d,j,valid,status,xoff,xpx,entry,
            aO,rO,rH,rL,rC,Cff,dates,ids,t_test,t_last,
            start_cash=1300000.0,max_positions=10,slip=args.slip,fee=FEE)
        cs=curve_stats(eqp,td_port)
        curves_port[label]=eqp
        wins=float((trp["net_ret"]>0).mean()) if len(trp) else np.nan
        ev=float(trp["net_ret"].mean()) if len(trp) else np.nan
        med=float(trp["net_ret"].median()) if len(trp) else np.nan
        pf=(float(trp.loc[trp["net_ret"]>0,"net_ret"].sum())/
            abs(float(trp.loc[trp["net_ret"]<0,"net_ret"].sum()))) if len(trp) and (trp["net_ret"]<0).any() else np.nan
        port_rows.append({
            "cohort":label,"signals":int(len(sig)),"closed_trades":int(len(trp)),
            "open_positions_end":open_end,"win_rate":wins,"ev":ev,"median":med,"pf":pf,
            "ending_nav":float(eqp[-1]),"total_return":cs["total"],"cagr":cs["cagr"],
            "max_dd":cs["mdd"],"avg_exposure":expp
        })
        all_trades.append(trp)

    # benchmarks on the identical 2025 window
    if len(j50):
        s50=Cff[t_test:t_last+1,j50[0]]
        if np.isfinite(s50[0]) and s50[0] > 0:
            b50eq=1300000.0*(s50/s50[0])*(1-args.slip)*(1-FEE)
            curves_port["0050_total_return"]=b50eq
            b50s=curve_stats(b50eq,td_port)
            port_rows.append({"cohort":"0050_total_return","signals":0,"closed_trades":0,"open_positions_end":1,
                              "win_rate":np.nan,"ev":np.nan,"median":np.nan,"pf":np.nan,
                              "ending_nav":float(b50eq[-1]),"total_return":b50s["total"],"cagr":b50s["cagr"],
                              "max_dd":b50s["mdd"],"avg_exposure":1.0})
    if args.r10_equity and os.path.exists(args.r10_equity):
        ...
        rq=rq[(rq["date"]>=td_port[0])&(rq["date"]<=td_port[-1])].dropna(subset=["date"]).sort_values("date")
        if ec and len(rq)>5:
            rv=to_num(rq[ec]).values
            rd=pd.DatetimeIndex(rq["date"])
            rs=curve_stats(rv/rv[0],rd)
            port_rows.append({"cohort":"R10_MAX_formal","signals":0,"closed_trades":0,"open_positions_end":np.nan,
                              "win_rate":np.nan,"ev":np.nan,"median":np.nan,"pf":np.nan,
                              "ending_nav":float(1300000.0*(rv[-1]/rv[0])),"total_return":rs["total"],
                              "cagr":rs["cagr"],"max_dd":rs["mdd"],"avg_exposure":np.nan})

    port_summary=pd.DataFrame(port_rows)
    port_summary.to_csv(os.path.join(port_dir,"portfolio_summary.csv"),index=False,encoding="utf-8-sig")
    pd.DataFrame(curves_port).to_csv(os.path.join(port_dir,"portfolio_equity_curves.csv"),index=False,encoding="utf-8-sig")
    if all_trades:
        pd.concat(all_trades,ignore_index=True).to_csv(os.path.join(port_dir,"portfolio_trades.csv"),index=False,encoding="utf-8-sig")
    meta={"start_cash":1300000,"max_positions":10,"position_target":"10% prior-close NAV",
          "whole_shares":True,"shared_cash":True,"t_plus_1":True,"same_stock_no_duplicate":True,
          "exit_rule":"original Route2 +10% / 1.5ATR / 30d; stop/expiry next-open",
          "limit_down_delay":"locked next-open sell is postponed",
          "cohorts":["daily D10","daily Top2%","P>=85"],
          "warning":"2025 has already been inspected; this is diagnostic research, not a fresh OOS claim."}
    json.dump(meta,open(os.path.join(port_dir,"portfolio_meta.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=2)
    log("\n── Formal Portfolio Baseline ──")
    log(port_summary.to_string(index=False))
