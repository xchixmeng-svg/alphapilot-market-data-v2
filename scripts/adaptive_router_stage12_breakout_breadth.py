...
# causal breadth threshold: prior 252d percentile, no current/future threshold fitting
...
# fixed, economically interpretable variants; selection only on 2016-18
variants={
...
def benchmark(year):
    # 0050 if present; buy first close, hold to last close. benchmark gate is explicit.
...
def run(year,name,maskfn):
    ...
    for i,dt in enumerate(dates):
        day=by[dt]
        # fixed 20-trading-day exit; no Stage10 retuning
        ...
        if i>0:
            ...
            for _,r in sigby.get(prev,pd.DataFrame()).iterrows():
                ...
                # precommitted T+1 limit = T close * 1.005; fill only if actual low touches limit
...
# Discovery gate explicitly requires beating benchmark over 2016-18 aggregate majority + acceptable mechanics.
...
# freeze best discovery candidate by alpha_mean only among passers, then OOS gates demand benchmark outperformance.
