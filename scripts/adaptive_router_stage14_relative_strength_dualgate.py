...
ROOT=Path('reference_bundle'); OUT=Path('adaptive_router_stage14'); OUT.mkdir(exist_ok=True)
...
for n in [60,120]:raw[f'ma{n}']=g.adj_close.transform(lambda s,n=n:s.rolling(n,min_periods=n).mean())
raw['prior60']=g.adj_high.transform(lambda s:s.shift(1).rolling(60,min_periods=60).max());raw['prior120']=g.adj_high.transform(lambda s:s.shift(1).rolling(120,min_periods=120).max());raw['break60']=raw.adj_close/raw.prior60-1;raw['break120']=raw.adj_close/raw.prior120-1
raw['amt20']=g.amount.transform(lambda s:s.rolling(20,min_periods=20).mean());raw['liq_pct']=raw.groupby('date').amt20.rank(pct=True)

# benchmark state is causal T-close information only
b=raw[raw.code=='0050'][['date','adj_close','r20','r60','r120','ma120']].drop_duplicates('date').sort_values('date').rename(columns={c:'b_'+c for c in ['adj_close','r20','r60','r120','ma120']})
raw=raw.merge(b,on='date',how='left');raw['rel20']=raw.r20-raw.b_r20;raw['rel60']=raw.r60-raw.b_r60;raw['rel120']=raw.r120-raw.b_r120
raw['market_on']=(raw.b_adj_close>raw.b_ma120)&(raw.b_r60>0)
base=raw.code.str.fullmatch(r'[1-9]\d{3}')&(raw.liq_pct>=.35)&(~raw.action_recent)&raw.market_on&(raw.adj_close>raw.ma120)&(raw.r60>0)

# Fixed economic variants; no future labels and no 2019/2020 selection.
def mask_score(d,name):
    if name=='RS60':m=(d.rel60>0);s=d.rel60
    elif name=='RS60_120':m=(d.rel60>0)&(d.rel120>0);s=d.rel60+d.rel120
    elif name=='RS_BREAK120':m=(d.rel60>0)&(d.break120>=0);s=d.rel60+2*d.break120
    else:m=(d.rel120>0)&(d.r10<0)&(d.adj_close>d.ma60);s=d.rel120-d.r10
    return m,s
VARIANTS=['RS60','RS60_120','RS_BREAK120','RS_PULLBACK']

def bench(y):
    q=raw[(raw.code=='0050')&(raw.date.dt.year==y)].sort_values('date');return float(q.adj_close.iloc[-1]/q.adj_close.iloc[0]-1)

def sim(y,name):
    d=raw[raw.date.dt.year==y].copy();dates=sorted(d.date.unique());by={dt:x.set_index('code',drop=False) for dt,x in d.groupby('date')};m,s=mask_score(d,name);c=d[base.loc[d.index]&m].copy();c['score']=s.loc[c.index];c['score_pct']=c.groupby('date').score.rank(pct=True);c=c[c.score_pct>=.80];sig={dt:x.sort_values(['score_pct','score','liq_pct'],ascending=False) for dt,x in c.groupby('date')}
    cash=INIT;pos={};pend=[];pnls=[];navs=[]
    for i,dt in enumerate(dates):
        ...
        for code,p in list(pos.items()):
            if i-p['entry_i']>=HOLD and code in day.index:
                px=float(day.loc[code].open)*(1-SLIP);pro=p['shares']*px*(1-FEE-TAX);cash+=pro;pnls.append(pro-p['cost']);del pos[code]
        for o in pend:
            code=o['code']
        ...
        pend=[];mv=sum(p['shares']*float(day.loc[c0].close) for c0,p in pos.items() if c0 in day.index);navs.append(cash+mv);slots=MAX_POS-len(pos)
        if slots>0:
...
m={'stage':'14_relative_strength_dualgate','market_refetch':False,'formal_r10_modified':False,'uses_2021_2025':False,'future_labels_used':False,'benchmark':'0050','benchmark_gate_required':True,'market_gate':'0050_adj_close_gt_ma120_and_r60_gt_0','stock_gate':'adj_close_gt_ma120_and_r60_gt_0','variants':VARIANTS,'selected':selected,'oos1_pass':o1,'oos2_pass':o2,'oos1_used_for_selection':False,'oos2_used_for_selection':False,'corporate_action_method':'inferred_bridge_0p82_1p18_same_as_stage7','t_plus_1':True,'integer_shares':True,'shared_cash':True};(OUT/'manifest.json').write_text(json.dumps(m,indent=2));print('STAGE14_COMPLETE');print(json.dumps(m,indent=2));print('\nDISCOVERY');print(disc.to_string(index=False));print('\nYEARLY');print(r.to_string(index=False))