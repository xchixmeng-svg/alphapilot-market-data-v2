...
These rules are NOT optimizer parameters. Any factory may search alpha logic,
ranking weights, filters, exits, and regime logic only inside these boundaries.
A run that violates this contract is invalid regardless of CAGR.
"""
...
# ---------- transaction costs; always applied to RAW executable prices ----------
...
def harden_portfolio_params(p: dict[str, Any]) -> dict[str, Any]:
    """Overwrite all optimizer-owned capital knobs with the locked R10 limits."""
    q = dict(p)
...
def size_shares(target_cash: float, raw_limit: float, avg_vol20: float, base_target_cash: float) -> tuple[int, str]:
    """Board-lot first; odd lot only for genuinely high-priced names.

    IMPORTANT: an odd lot is allowed only when one 1,000-share board lot exceeds
    the intended BASE position target. A tiny residual capacity may not create a
    1-share/2-share low-price position; that order is skipped instead.
    """
    ...
    one_lot_cash = float(raw_limit) * BOARD_LOT
    oddlot_allowed = one_lot_cash > float(base_target_cash) + 1e-9

    if oddlot_allowed:
        shares = int(math.floor(float(target_cash) / float(raw_limit) + 1e-12))
        return max(0, min(shares, liq_shares)), "HIGH_PRICE_ODDLOT"

    lots_by_cash = int(math.floor(float(target_cash) / one_lot_cash + 1e-12))
...
def can_emit_sell(hold_days: int) -> bool:
    """No exit order may be CREATED before 3 completed held trading sessions."""
...
def contract_dict() -> dict[str, Any]:
    return {
        ...
        "common_cash_pool": COMMON_CASH_POOL,
...
def validate_hard_params(p: dict[str, Any]) -> None:
    checks = {
        "max_positions": MAX_DISTINCT_STOCKS,
        "max_single": MAX_SINGLE_STOCK_NAV,
        ...
        "dd_mult3": DD_MULT3,
        "force_dd": FORCE_DD,
        "force_target_exposure": FORCE_TARGET_EXPOSURE,
        "force_no_buy": FORCE_NO_BUY_DAYS,
