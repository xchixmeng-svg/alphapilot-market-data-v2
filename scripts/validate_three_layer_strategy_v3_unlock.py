...
OUT = ROOT / "artifacts" / "three_layer_strategy_v3_unlock"
...
def simulate(x: pd.DataFrame, enabled: tuple[str,...], label: str):
    ...
    for day_i,day in enumerate(days):
        ...
        pending_buys=[]
        nav=cash+sum(p["shares"]*last[c] for c,p in pos.items())
        equity_peak=max(equity_peak,nav); true_dd=nav/equity_peak-1
        risk_peak=max(risk_peak,nav); risk_dd=nav/risk_peak-1
        vals.append((day,nav,cash,len(pos),true_dd))
        ...
        # V3 unlock supplement.  The active guard tracks its own trough.  On
        # release a new risk episode begins, while reported drawdown continues
        ...
        dd_events.append((day,true_dd,risk_dd,guard_level,len(pending_sells)))
        if guard_level>=12 or day not in idx.index: continue
        mr=idx.loc[day]
        ...
        for layer in enabled:
            if guard_level>=9 and layer=="short": continue
            if guard_level>=9 and layer=="swing" and any(p["layer"]=="swing" for p in pos.values()): continue
        ...
        pending_buys=[]
        layer_remaining={layer:max(0.0,nav*.50-sum(p["shares"]*last[c] for c,p in pos.items()
            if p["layer"]==layer)) for layer in ("short","swing")}
        for c,l,_,box,support in chosen:
            ...
            if rr<1.5:
                ...
                continue
            budget=each
            if guard_level>=6 and l in layer_remaining:
                budget=min(budget,layer_remaining[l]); layer_remaining[l]-=budget
            if budget<=0: continue
            pending_buys.append(dict(code=c,layer=l,limit=limit,budget=budget,
    ...
    return result,curve,pd.DataFrame(ledger),pd.DataFrame(split_events),pd.DataFrame(rejected),pd.DataFrame(guard_events)
...
def main():
    ...
    for layers,label in configs:
        r,c,t,s,j,g=simulate(x,layers,label); results.append(r); ledgers.append(t); curves.append(c)
        split_audits.append(s); rejected_orders.append(j); guard_logs.append(g)
        t.to_csv(OUT/f"trades_{label}.csv",index=False); g.to_csv(OUT/f"guard_events_{label}.csv",index=False)
    ...
    contract_checks=audit(x,results,ledgers,curves,split_audits,rejected_orders)
    post=[]
    for i,(_,label) in enumerate(configs[:3]):
        layer=configs[i][0][0]; t=ledgers[i]
        buys=t[t.side.eq("BUY")].copy() if not t.empty else pd.DataFrame()
        if not buys.empty: buys["year"]=pd.to_datetime(buys.execute_date).dt.year
        for year in range(2022,2027):
            post.append(dict(layer=layer,year=year,new_buys=int((buys.year.eq(year)).sum()) if not buys.empty else 0))
    post_df=pd.DataFrame(post); post_df.to_csv(OUT/"post_2021_new_trade_audit.csv",index=False)
    guard_frames=[g.assign(strategy=configs[i][1]) for i,g in enumerate(guard_logs) if not g.empty]
    ...
    unlock_checks={
        ...
        "every_unlock_satisfies_6pct_or_20d_plus_dd9":bool(unlock_valid),
        "no_automatic_position_restoration":bool(all(t.empty or not t.reason.astype(str).str.contains("restore|reopen",case=False,regex=True).any() for t in ledgers)),
        "each_layer_has_new_buy_in_2022_2026":bool(post_df.groupby("layer").new_buys.sum().gt(0).all()),
        "each_layer_each_year_has_new_buy_2022_2026":bool(post_df.new_buys.gt(0).all()),
        "short_complete_trades_at_least_80":bool(short_complete>=80),
    }
    (OUT/"unlock_audit.json").write_text(json.dumps(unlock_checks,indent=2),encoding="utf-8")
    report={"period":{"warmup":"2020","train":"2021-2023","test":"2024-2025","blind":"2026-01-01..2026-08-28"},
      "version":"V3 plus drawdown-guard unlock supplement",
