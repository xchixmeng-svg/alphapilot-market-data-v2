...
direction: SUPPORTIVE / CONTRADICTORY / NEUTRAL / MIXED / NOT_INTERPRETABLE, relative to a >=10% upside opportunity. Apply ordinary financial literacy: an extremely high PE with no growth story present is not "reasonable" merely because no peer benchmark was supplied -- extreme absolute values can still be CONTRADICTORY or NOT_INTERPRETABLE, but you must say explicitly in `limitations` that no benchmark was supplied and that you are relying on the absolute magnitude alone.
...
limitations: State explicitly, whenever applicable:
  - "no historical or peer valuation benchmark supplied -- cheap/expensive cannot be asserted" (whenever benchmark_available is false and the family is valuation-related)
...
If a previous_attempt field is present in your input, it contains your own prior response and the exact contract errors it violated. Fix precisely those errors; do not otherwise change parts of your answer that were not flagged.
