...
# Critical module-global binding: v12.mark_baseline_frontier reads v12.BASELINE_DIR.
...
def choose_route(prior,row):
    specs=[('STRATEGY_STATE',(prior.strategy==row.strategy)&(prior.market_state==row.market_state)),('STRATEGY',prior.strategy==row.strategy),('GLOBAL',pd.Series(True,index=prior.index))]
...
def neighbor_evidence(sub,row):
    X=sub[FEATURES].astype(float); med=X.median(); q1=X.quantile(.25); q3=X.quantile(.75); scale=(q3-q1).replace(0,np.nan).fillna(X.std()).replace(0,1.0).fillna(1.0)
    z=(X-med)/scale; zr=(pd.Series({f:float(row[f]) for f in FEATURES})-med)/scale; dist=np.sqrt(((z-zr)**2).sum(axis=1))
    n=len(sub); k=min(20,max(8,int(round(np.sqrt(n)*3)))); nn=sub.loc[dist.nsmallest(k).index].copy()
    return nn,float(nn.path_uplift.mean()),float((nn.path_uplift>0).mean()),float(nn.path_uplift.median()),k
...
def main():
    ...
    expmap=px.groupby('date')['r7_exposure'].first().to_dict() if 'r7_exposure' in px.columns else {}; pairs['market_exposure']=[float(expmap.get(int(d),np.nan)) for d in pairs.date]; pairs['market_state']=[v12.exposure_state(x) for x in pairs.market_exposure]
    ...
    pairs['upper_shadow']=[shadow(d,c,s) for d,c,s in zip(pairs.date,pairs.upper_code,pairs.strategy)]; pairs['lower_shadow']=[shadow(d,c,s) for d,c,s in zip(pairs.date,pairs.lower_code,pairs.strategy)]
    pairs['path_uplift']=pairs.lower_shadow-pairs.upper_shadow; pairs['path_hit']=np.where(pairs.path_uplift.notna(),(pairs.path_uplift>0).astype(float),np.nan)
    train=pairs[pairs.execution_frontier & pairs.path_uplift.notna()].copy(); print(f'V13 stage=shadow_labels done train={len(train)}',flush=True)
    scored=[]; audit=[]
    for yr in [2022,2023,2024,2025]:
        prior=train[train.year<yr].copy(); cur=pairs[pairs.year==yr].copy(); cur['meta_allow']=False; cur['route']='ABSTAIN'; cur['neighbor_n']=0; cur['neighbor_mean_uplift']=np.nan; cur['neighbor_hit_rate']=np.nan; cur['neighbor_median_uplift']=np.nan
        for idx,row in cur.iterrows():
            route,sub=choose_route(prior,row)
            if route=='ABSTAIN': continue
            nn,mean,hit,med_u,k=neighbor_evidence(sub,row); allow=bool((row.ret_margin>0) and (row.fail_margin>0) and (mean>0) and (hit>=0.60))
        ...
        scored.append(cur)
    scored=pd.concat(scored,ignore_index=True); scored.to_csv(RUN_ROOT/'AI_SLOT_PERMISSIONS.csv',index=False); pairs.to_csv(RUN_ROOT/'AI_V13_ROUTE_PAIRS.csv',index=False); train.to_csv(RUN_ROOT/'AI_V13_PATH_TRAINING.csv',index=False); pd.DataFrame(audit).to_csv(RUN_ROOT/'AI_V13_DECISION_AUDIT.csv',index=False); pred.to_csv(RUN_ROOT/'AI_OOS_PREDICTIONS.csv',index=False); diag.to_csv(RUN_ROOT/'AI_OOS_MODEL_DIAGNOSTICS.csv',index=False)
    locked=v2.mod.LOCKED.read_text(encoding='utf-8'); VARIANT_DIR.mkdir(parents=True,exist_ok=True); v2.link_inputs(VARIANT_DIR); runner=VARIANT_DIR/'runner.py'; runner.write_text(build_runner(locked),encoding='utf-8'); v2.run_py_verbose(runner,VARIANT_DIR,'execution.log')
    comp=pd.DataFrame([v2.mod.summarize(BASELINE_DIR,'BASELINE_R10'),v2.mod.summarize(VARIANT_DIR,'AI_CONTEXT_NEIGHBOR')]); b=comp.iloc[0]; r=comp.iloc[1]
    comp['cagr_delta_pp']=(comp.cagr-b.cagr)*100; comp['dd_improvement_pp']=(comp.max_drawdown-b.max_drawdown)*100; comp['pf_delta']=comp.pnl_profit_factor-b.pnl_profit_factor; comp['win_delta_pp']=(comp.win_rate-b.win_rate)*100; comp.to_csv(RUN_ROOT/'AI_V13_COMPARISON.csv',index=False)
    gate={'cagr_better':bool(r.cagr>b.cagr),'pf_not_worse':bool(r.pnl_profit_factor>=b.pnl_profit_factor),'dd_not_worse_over_1pp':bool(r.max_drawdown>=b.max_drawdown-0.01)}; gate['pass']=all(gate.values()); (RUN_ROOT/'AI_V13_SUCCESS_GATE.json').write_text(json.dumps(gate,indent=2),encoding='utf-8')
    print('=== V13 INTERVENTIONS ==='); print(scored.groupby(['year','route']).meta_allow.agg(['count','sum']).reset_index().to_string(index=False)); print('\n=== V13 PORTFOLIO ==='); print(comp.to_string(index=False)); print('\n=== SUCCESS GATE ==='); print(json.dumps(gate,indent=2))
