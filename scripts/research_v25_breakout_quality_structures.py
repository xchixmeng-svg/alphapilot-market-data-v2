...
# trigger-v25-matrix
HYPOTHESES={
 'breakout_retest_recovery':(25,3),'breakout_quiet_absorption':(25,3),'breakout_sponsor_persistence':(25,3),
 'breakout_residual_acceleration':(20,3),'breakout_early_stage':(25,3),'breakout_liquidity_quality':(20,3),
 'breakout_compression_release':(30,2),'breakout_soft_breadth_tailwind':(20,3),'breakout_relative_strength_confirm':(20,3),
 'breakout_flow_then_price':(25,3),'breakout_delayed_confirmation':(20,3),'liquidity_sponsor_breakout':(25,3),
}
...
def make_signals(d,name):
 ...
 lag5_flow=pct(lag(d,'flow5_pr',5),d.date); lag5_acc=pct(lag(d,'flow_accel_pr',5),d.date); lag5_r20=pct(lag(d,'r20_pr',5),d.date)
 lag3_dist=lag(d,'dist_ma20',3); lag3_r5=pct(lag(d,'r5_pr',3),d.date); lag10_vol=pct(lag(d,'vol20_pr',10),d.date)
 base=(d.flow_accel_pr>=.62)&(d.flow20_pr>=.52)&(d.close>=10)&d.amount20.notna(); quality=(d.amount20_pr>=.35)&(d.vol20_pr<=.97)
 breakout=(r20p>=.62)&(d.r5_pr>=.54)&(d.dist_ma20.between(0,.15))&(d.aclose>=d.ma60*.98); not_ext=d.dist_ma20.between(-.08,.16)
 if name=='breakout_retest_recovery':
  mask=base&(r20p>=.62)&(lag3_dist.between(-.045,.035))&(d.dist_ma20.between(.005,.11))&(d.r5_pr>=.58)&(d.flow5_pr>=.54); score=.22*d.flow_accel_pr+.16*d.flow20_pr+.24*r20p+.14*d.r5_pr+.10*d.flow5_pr+.14*p['breadth_up']
 elif name=='breakout_quiet_absorption':
  mask=base&breakout&(d.vol20_pr<=.58)&(lag10_vol<=.62); score=.22*d.flow_accel_pr+.15*d.flow20_pr+.24*r20p+.16*(1-d.vol20_pr)+.11*d.amount20_pr+.12*p['calm']
 elif name=='breakout_sponsor_persistence':
  mask=breakout&(d.flow_accel_pr>=.60)&(d.flow20_pr>=.56)&(lag5_flow>=.52)&(lag5_acc>=.50)&quality; score=.20*d.flow_accel_pr+.14*d.flow20_pr+.14*lag5_flow+.12*lag5_acc+.24*r20p+.10*d.r5_pr+.06*p['flow_breadth']
 elif name=='breakout_residual_acceleration':
  mask=base&breakout&(lag5_r20.between(.45,.75))&(r20p>=.68)&(r5p>=.58); score=.20*d.flow_accel_pr+.13*d.flow20_pr+.29*r20p+.16*r5p+.10*(1-lag5_r20)+.12*d.amount20_pr
 elif name=='breakout_early_stage':
  mask=base&breakout&(r60p.between(.40,.72))&(d.r60_pr<=.82)&not_ext; score=.22*d.flow_accel_pr+.15*d.flow20_pr+.27*r20p+.12*d.r5_pr+.12*(1-r60p)+.12*d.amount20_pr
 elif name=='breakout_liquidity_quality':
  mask=base&breakout&(d.amount20_pr>=.72)&(d.vol20_pr<=.78); score=.20*d.flow_accel_pr+.14*d.flow20_pr+.25*r20p+.19*d.amount20_pr+.12*(1-d.vol20_pr)+.10*p['flow_breadth']
 elif name=='breakout_compression_release':
  prior_near=(lag3_dist.abs()<=.045)&(lag3_r5<=.68); mask=base&breakout&prior_near&(d.vol20_pr<=.60)&(d.flow5_pr>=.52); score=.20*d.flow_accel_pr+.14*d.flow20_pr+.25*r20p+.12*d.r5_pr+.13*(1-d.vol20_pr)+.08*d.flow5_pr+.08*p['calm']
 elif name=='breakout_soft_breadth_tailwind':
  mask=base&breakout&not_ext; score=.20*d.flow_accel_pr+.14*d.flow20_pr+.24*r20p+.11*d.r5_pr+.10*d.amount20_pr+.11*p['flow_breadth']+.10*p['breadth_up']
 elif name=='breakout_relative_strength_confirm':
  mask=base&breakout&(r5p>=.56)&(r20p>=.64)&(r60p>=.50)&(r60p<=.84); score=.18*d.flow_accel_pr+.13*d.flow20_pr+.18*r5p+.27*r20p+.12*r60p+.12*d.amount20_pr
 elif name=='breakout_flow_then_price':
  mask=(lag5_acc>=.58)&(lag5_flow>=.55)&breakout&(d.flow_accel_pr>=.55)&quality; score=.16*lag5_acc+.14*lag5_flow+.20*d.flow_accel_pr+.24*r20p+.14*d.r5_pr+.12*d.amount20_pr
 elif name=='breakout_delayed_confirmation':
  mask=base&breakout&(lag3_r5>=.52)&(d.r5_pr>=lag3_r5)&(d.flow5_pr>=.55)&not_ext; score=.20*d.flow_accel_pr+.14*d.flow20_pr+.25*r20p+.15*d.r5_pr+.10*lag3_r5+.08*d.flow5_pr+.08*d.amount20_pr
 else:
  mask=(d.amount20_pr>=.70)&(d.flow_accel_pr>=.60)&(d.flow20_pr>=.54)&breakout&(d.vol20_pr<=.86); score=.20*d.flow_accel_pr+.14*d.flow20_pr+.24*r20p+.17*d.amount20_pr+.11*d.r5_pr+.08*(1-d.vol20_pr)+.06*p['flow_breadth']
