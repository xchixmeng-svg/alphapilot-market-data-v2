...
def validate(pkt,o):
    ...
    if actionable:
        ...
        if not (0.65*current <= e["ideal_low"] <= 1.30*current and 0.65*current <= e["ideal_high"] <= 1.30*current):
            raise ValueError("entry zone implausibly far from current")
    ...
    # Semantic guard for explicitly unavailable families.
    txt=json.dumps(o,ensure_ascii=False).lower()
    forbidden={
      "eps_revisions":["eps revision","eps revisions","eps上修","eps 預估上修"],
      "analyst_consensus":["analyst consensus","consensus estimate","分析師共識"],
      "industry_pricing":["industry pricing","spot price","產業報價","現貨價"],
      "inventory_supply_demand":["inventory destocking","supply shortage","供不應求","庫存去化"],
      "broad_news_semantics":["news sentiment","媒體情緒"]
    }
...
def call(pkt,repair=None):
    response_schema={
      ...
      "hypothesis_type":"string","hypothesis":"string",
      "primary_evidence_ids":"list of supplied available evidence IDs only",
      "secondary_evidence_ids":"list of supplied available evidence IDs only",
      "counter_evidence_ids":"list of supplied available IDs or missing:<family>",
      "bull_thesis":"string","bear_thesis":"string","invalidation":"string","decision_reason":"string",
...
def main(shard):
    path=Path(f"prepared/cases_shard_{shard}.jsonl")
    ...
    for pkt in cases:
        ...
        for attempt in [1,2]:
            try:
                obj,elapsed=call(pkt,err); break
            except Exception as e:
                err=repr(e)
        if obj is None:
            errors.append({"case_id":pkt["case_id"],"date":pkt["decision_date"],"code":pkt["code"],"error":err})
            print("ERROR",errors[-1],flush=True); continue
        row={"case_id":pkt["case_id"],"date":pkt["decision_date"],"code":pkt["code"],"elapsed_seconds":elapsed,
             "p_hit10_h120":pkt["numerical_reference_read_only"]["p_hit10_h120"],**obj}
        rows.append(row)
