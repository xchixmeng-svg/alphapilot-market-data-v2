...
TASKS={
  "short_5_3_10": (0.05,-0.03,10),
  "mid_10_5_20": (0.10,-0.05,20),
  "long_20_10_60": (0.20,-0.10,60),
}
...
def aggregate(inp):
    ...
    if len(rows)!=3: raise RuntimeError(f"expected 3 summaries, got {len(rows)}")
    out={
      "layer":"V6.2 Core Numerical Opportunity Engine - Prototype A",
    ...
    (ROOT/"V6_2_CORE_NUMERICAL_PROTOTYPE_A_SUMMARY.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    pd.DataFrame([{
      "task":r["task"],"horizon":r["horizon"],
      **r["metrics"],**{f"holdout_{k}":v for k,v in r["class_rate_holdout"].items()}
    } for r in rows]).to_csv(ROOT/"V6_2_CORE_NUMERICAL_PROTOTYPE_A_METRICS.csv",index=False)
