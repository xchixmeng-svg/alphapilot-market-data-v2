"""Causal A/B test: reanalyse V3 large candidates after a stabilized base."""
...
OUT = ROOT / "artifacts" / "large_pullback_stable_base"
OUT.mkdir(parents=True, exist_ok=True)


def build_pullback_signals(x: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame]:
    """After a >=10% decline, require a causal ten-day base and rerun analysis."""
            ...
            for age in range(1, 61):
                ...
                if drawdown < -.30:
                    audit_rows.append(dict(code=code, raw_signal_date=x.at[start_idx, "date"],
                        outcome="cancel_over_30pct", end_date=x.at[k, "date"], peak=peak,
                        drawdown=drawdown))
                    break
                if age < 10 or drawdown > -.10:
                    continue
                stable_ids=ids[j-9:j+1]; ck=k
                stable_closes=x.loc[stable_ids,"adj_close"].astype(float)
                stable_volumes=x.loc[stable_ids,"volume"].astype(float)
                first5=stable_closes.iloc[:5]; last5=stable_closes.iloc[5:]
                cclose=close
                tests={
                    "drawdown_at_least_10pct":drawdown<=-.10,
                    "ten_day_close_range_at_most_6pct":float(stable_closes.max()/stable_closes.min()-1)<=.06,
                    "second_half_low_not_break_first_half_by_2pct":float(last5.min())>=float(first5.min())*.98,
                    "close_at_least_1pct_above_base_low":cclose>=float(stable_closes.min())*1.01,
                    "ten_day_volume_contracted":float(stable_volumes.mean())<=float(x.at[ck,"vol20"]),
                ...
                if all(tests.values()):
                    ...
                    audit_rows.append(dict(code=code, raw_signal_date=x.at[start_idx, "date"],
                        ...
                        peak=peak, pullback_date=x.at[stable_ids[0], "date"],drawdown=drawdown,
                        rejection_reasons=""))
                    finished = True
                else:
                    audit_rows.append(dict(code=code,raw_signal_date=x.at[start_idx,"date"],
                ...
                audit_rows.append(dict(code=code, raw_signal_date=x.at[start_idx, "date"],
                    outcome="expired_60d", end_date=x.at[ids[min(start+60, len(ids)-1)], "date"],
                    peak=peak))
...
def main():
    ...
    configs=[]
    for variant, large_signal in (("v3_original", original_large), ("pullback_stable_base", pullback)):
    ...
    checks={
        ...
        "pullback_at_least_10pct":bool((signal_audit.query("outcome == 'reanalysis_passed'").drawdown<=-.10).all()),
        "base_uses_ten_prior_days":bool((pd.to_datetime(signal_audit.query("outcome == 'reanalysis_passed'").end_date)>
    ...
    report={"test":"large signal, >=10% pullback, ten-day stable base, full reanalysis, then T+1",
        "watch_days":60,"stabilization_days":10,"results":results,
