...
def main()->None:
    ap=argparse.ArgumentParser();ap.add_argument("--scenario",required=True,choices=ALL);args=ap.parse_args()
    try:
        r=run_one(args.scenario);print(json.dumps(r,ensure_ascii=False,indent=2,allow_nan=False))
    except Exception as exc:
        payload={"status":"ERROR","scenario":args.scenario,"engine_version":bt.VERSION,"generated_at":datetime.now().astimezone().isoformat(),"error":str(exc)}
        out=bt.OUT_ROOT/"latest"/args.scenario;out.mkdir(parents=True,exist_ok=True)
        (out/"summary.json").write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
        print(json.dumps(payload,ensure_ascii=False,indent=2));raise
